...
func RangeCourseCreate(c *gin.Context)  {
	...
	set := myRedis.RedisService.Set(res.Data.CourseID, requestParams.Cap, myRedis.RedisTimeOutForKeep)
	fmt.Println(set.Err())
	c.JSON(http.StatusOK,res)
	return
}
...
func RangeSchedule(c *gin.Context)  {
	...
	for k,_ := range requestParams.TeacherCourseRelationShip {
		...
		if findByCwc(k,haveInterest,haveCourse,used,ans){
			sum++
		}
	}
...
func findByCwc(teacherId string, interest map[string]int, course map[string]string, used map[string]int, ans map[string]string) bool {
	for courseId,_ := range course {
		_,ok1:=interest[teacherId+"->"+courseId]
		_,ok2:=used[courseId]
		if ok1 && !ok2{
			used[courseId]=1
			if _,ok:=ans[courseId];!ok || findByCwc(ans[courseId],interest,course,used,ans){
